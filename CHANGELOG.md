...
Toutes les modifications notables de BlackOut iOS sont documentées ici.
...
- Structure XcodeGen (`project.yml`) : cible `BlackOutCore` (framework logique
  pure), `BlackOut` (app SwiftUI), `BlackOutTests` (XCTest sur le core).
- Portage fidèle des règles Borderland depuis la référence web
  (`black-out/src/core/borderland.ts`) : deck 52 cartes, As = pénalité
...
- `ContentPack` Codable aligné sur `blackout-content/schema/content.schema.json`.
...
- Thème Neo-Tokyo Borderland porté depuis `blackout-content/tokens/tokens.json`.
...
- CI GitHub Actions (`macos-15`) : génération XcodeGen, build et tests
  `BlackOutTests` sur simulateur iPhone 16.
