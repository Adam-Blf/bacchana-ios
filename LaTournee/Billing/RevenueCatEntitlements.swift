...
/// `PREMIUM_ENTITLEMENT_ID` (`la-taverne/src/lib/billing.ts`).
///
/// Product rename (v0.15.0, Meskova -> La Tournee, display "La Tournée"): renamed to
/// "La Tournee Pro" (no accent, exact RevenueCat dashboard id) - unlike a normal rename, the
/// entitlement id itself moves this time. Safe because the app is not yet published: there are
/// no existing subscribers to migrate.
private let premiumEntitlementID = "La Tournee Pro"
...
/// `EntitlementProviding` and `LaTourneeCore.PremiumPackage`, never RevenueCat types directly.
