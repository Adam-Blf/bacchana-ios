...
/// qu'un `Player` (LaTourneeCore) n'existe. `Codable` pour la persistance
...
final class AppState: ObservableObject {
    ...
    /// analytics (voir `LaTourneeCore.Player.gender` / `.relationship`).
    ...
    // Prefix renamed from "lataverne." to "meskova." (v0.14.0), now "latournee." (v0.15.0).
    ...
    private let playerNamesKey = "latournee.playerNames"
    private let playerAttributesKey = "latournee.playerAttributes"
    private let themeModeKey = "latournee.themeMode"
    private let analyticsConsentKey = "latournee.analyticsConsent"
