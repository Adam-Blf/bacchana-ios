...
/// qu'un `Player` (BacchanaCore) n'existe. `Codable` pour la persistance
...
final class AppState: ObservableObject {
    ...
    /// analytics (voir `BacchanaCore.Player.gender` / `.relationship`).
    ...
    // Prefix renamed from "lataverne." to "meskova." (v0.14.0), then "latournee." (v0.15.0),
    // now "bacchana." (v0.16.0, final product name).
    ...
    // no `UserDefaults` written under any old prefix exist outside ephemeral CI simulators.
    private let playerNamesKey = "bacchana.playerNames"
    private let playerAttributesKey = "bacchana.playerAttributes"
    private let themeModeKey = "bacchana.themeMode"
    private let analyticsConsentKey = "bacchana.analyticsConsent"
